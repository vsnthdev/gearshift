...
const infoCard = cva({
    base: 'col-span-1 relative overflow-hidden md:aspect-square  rounded-2xl text-center flex flex-col space-y-2 justify-center items-center md:text-left',
    variants: {
        color: {
    ...
    return <div className={infoCard({ color })}>
        ...
        {icon && <div className='flex justify-center items-center'>
            <div className={cx(
                ...
                <span className='[&>svg]:w-5 md:[&>svg]:w-6 md:[&>svg]:h-6 '>
                    {icon}
                </span>
            </div>
        </div>}

        {value && <div className='flex flex-col items-center justify-center md:items-start z-30 md:absolute md:bottom-5 md:left-5'>
            {value && <h4 className='font-semibold md:text-xl md:font-bold'>{value}</h4>}
            {text && <p className='text-xs md:font-semibold md:text-base'>{text}</p>}
        </div>}
    </div>
}